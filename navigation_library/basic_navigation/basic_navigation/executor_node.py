...
class WaypointExecutor(Node):
    def __init__(self):
        ...
        self.current_heading = None  # Degrees
    ...
    def heading_callback(self, msg):
        self.current_heading = msg.data  # Degrees

    def calculate_and_publish_cmd_vel(self):
        if self.current_goal is None or self.current_position is None or self.current_heading is None:
            return  # Do nothing if we don't have goal, position, or heading
        ...
        goal_bearing_degrees = degrees(y) % 360

        # Calculate difference between current heading and goal bearing
        angle_diff = (goal_bearing_degrees - self.current_heading + 360) % 360
